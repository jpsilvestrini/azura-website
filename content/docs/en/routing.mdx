...
|--------|---------|----------|------------|------|------------|
| GET | Read resource | ❌ | ✅ | ✅ | Fetch data, list items |
| POST | Create resource | ✅ | ❌ | ❌ | Create records, submit forms |
| PUT | Update (full) | ✅ | ✅ | ❌ | Replace entire resource |
| PATCH | Update (partial) | ✅ | ❌ | ❌ | Modify specific fields |
| DELETE | Remove resource | ❌ | ✅ | ❌ | Delete records |
| HEAD | Headers only | ❌ | ✅ | ✅ | Check existence, metadata |
...
// Route: /users
GET /users        // ✅ Matches
GET /users/       // ❌ Doesn't match (trailing slash)
GET /users/123    // ❌ Doesn't match
```
...
```typescript
// Route: /users/:id
GET /users/123         // ✅ Matches (id = "123")
GET /users/abc         // ✅ Matches (id = "abc")
GET /users/abc-123     // ✅ Matches (id = "abc-123")
GET /users/123/posts   // ❌ Doesn't match
GET /users/            // ❌ Doesn't match (empty segment)
```
...
```typescript
// Route: /:category/:id
GET /books/123         // ✅ Matches (category="books", id="123")
GET /users/john        // ✅ Matches (category="users", id="john")
GET /books/123/pages   // ❌ Doesn't match (extra segment)
```
...
- `GET /api/users` → JSON with user list
...
```typescript
// ✅ Good: Most accessed routes first
...
// ❌ Slower: Complex validation in routes
...
```typescript
// ✅ Faster: Static routes
@Get("/api/status")
@Get("/api/health")
@Get("/api/version")

// ❌ Slower: Everything dynamic
@Get("/api/:action")
...
// ✅ Solution: Check controller prefix
...
const app = new AzuraClient();
applyDecorators(app, [UserController]); // ✅ Don't forget this

// 2. Wrong HTTP method
GET /api/users     // ✅ Defined
POST /api/users    // ❌ Not defined

// 3. Route order
@Get("/:id")       // ❌ This catches /me
@Get("/me")        // ❌ Never reached
```
...
```typescript
// ❌ Problem: Using wrong decorator
@Get("/search")
search(@Param("q") query: string) {}  // Param is for :param, not ?q=

// ✅ Solution: Use Query decorator
@Get("/search")
