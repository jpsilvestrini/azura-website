...
Distribua tráfego entre múltiplos servidores:
...
// Verificar saúde dos servidores a cada 30 segundos
setInterval(async () => {
...
function smartLoadBalancer({ req, res, next }) {
  ...
  if (healthyServers.length === 0) {
    return res.status(503).json({ error: 'Nenhum servidor disponível' });
  }
  
  // Escolher servidor saudável aleatório
...
Faça proxy de conexões WebSocket:
...
app.proxy('/api', 'http://localhost:4000', {
  ...
  onProxyReq: (proxyReq, req) => {
    ...
    // Transformar body se necessário
    if (req.body) {
...
async function getServiceUrl(serviceName) {
  ...
  if (result.length === 0) {
    throw new Error(`Nenhuma instância saudável de ${serviceName}`);
  }
  
  // Load balancing aleatório
...
// ❌ Ruim - Regex incorreta
app.proxy('/api/v1', 'http://localhost:4000', {
  pathRewrite: { '/api/v1': '' }  // Não vai funcionar corretamente
});
```

### 4. Adicione Rastreamento de Requisições
...
### 5. Preserve Headers Originais Quando Necessário

```javascript
app.proxy('/api', 'http://localhost:4000', {
  preserveHost: true,  // Mantém header Host original
  headers: {
...
```javascript
// 1. Verifique se o serviço alvo está rodando
// 2. Verifique regras de firewall
// 3. Implemente lógica de retry
...
app.proxy('/api', 'http://localhost:4000', {
  onError: async (err, req, res) => {
    if (err.code === 'ECONNREFUSED' && retries < maxRetries) {
      ...
      return; // Implementar lógica de retry aqui
    }
...
**Sintomas:** Requisições dando timeout
...
app.proxy('/api/reports', 'http://localhost:4000', {
  timeout: 60000,  // 60 segundos para relatórios
});
...
### Problema: Path Não Está Sendo Reescrito Corretamente

**Sintomas:** Erros 404 em requisições proxied

**Soluções:**

```javascript
// ✅ Correto: Use regex com ^
app.proxy('/api/v1', 'http://localhost:4000', {
  pathRewrite: { '^/api/v1': '' },
...
// ❌ Incorreto: Sem ^
app.proxy('/api/v1', 'http://localhost:4000', {
  pathRewrite: { '/api/v1': '' }  // Pode não funcionar corretamente
});
```

### Problema: Headers Não Sendo Encaminhados

**Sintomas:** Backend não recebe headers esperados
...
### Problema: Body Não Sendo Proxied

**Sintomas:** Requisições POST/PUT perdem o body

**Soluções:**

```javascript
// Certifique-se de que o body parser está habilitado
...
### Problema: Memory Leaks com Proxies de Longa Duração

**Sintomas:** Uso de memória cresce com o tempo
...
- **simple.js** - Configuração básica de proxy
- **microservices.js** - Gateway para múltiplos serviços
- **load-balancer.js** - Implementação de load balancing
- **circuit-breaker.js** - Padrão circuit breaker

## Tabela de Referência 📊

| Opção | Tipo | Padrão | Descrição |
|-------|------|--------|-----------|
| `target` | `string` | Obrigatório | URL do servidor alvo |
...
| `timeout` | `number` | `30000` | Timeout de requisição em ms |
...
| `logLevel` | `string` | `'none'` | Nível de log: none, info, debug |
| `onProxyReq` | `function` | `null` | Modificar requisição antes do proxy |
| `onProxyRes` | `function` | `null` | Modificar resposta após o proxy |
...
- [Configuração de CORS](/docs/pt/cors)
