...
// Usar
const rateLimit = createRateLimit(100, 60000); // 100 requisições por minuto
app.use(rateLimit);
```
...
// Tratamento de erro em nível de rota
...
Quando estiver pronto para adicionar segurança de tipos:

1. **Renomear arquivos** - `.js` → `.ts`
2. **Adicionar tipos** - Importe tipos do `azurajs`
3. **Decorators opcionais** - Use decorators para melhor organização
...
app.get("/data", (req, res, next) => {
  const logger = createLogger(); // Criado em cada requisição!
  next();
}, handler);
...
- [Exemplos JavaScript](https://github.com/azurajs/azura/tree/main/examples/servers)
- [Referência da API](https://azura.js.org/docs/pt)
